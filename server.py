...
def ScrapePage():
  ...
  if request.method == 'POST':
    ...
    def search_tweets():
      for i,tweet in enumerate(sntwitter.TwitterSearchScraper('%s' %searchquery).get_items()):
          if i>5000:
              break
    ...
    def user_tweets(username):
      print(pd.DataFrame(itertools.islice(sntwitter.TwitterUserScraper('"% s"' %searchquery).get_items(), 20)))
      df = pd.DataFrame(itertools.islice(sntwitter.TwitterUserScraper('"% s"' %searchquery).get_items(), 20))
      print(df.columns)
      #st.bar_chart()
    ...
    def tweets():
      print(pd.DataFrame(itertools.islice(sntwitter.TwitterSearchScraper('"% s"' %searchquery).get_items(), 20)))
